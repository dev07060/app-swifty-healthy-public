...
import { validateFoodPhoto } from '../../utils/metadataValidation';
import { styles } from './styles';

interface FoodUploadScreenProps {
    navigation: {
        goBack: () => void;
    };
}

export function FoodUploadScreen({ }: FoodUploadScreenProps) {
    ...
    const validation = useValidation(
        async (imageUri: string) => {
            const result = await validateFoodPhoto(imageUri);
            if (!result.isValid) {
                throw new Error(result.error || 'Photo validation failed');
            }
            return true;
        },
        {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const appError = ErrorHandlingUtils.handleProcessingError(
                error instanceof Error ? error : new Error('분석실패 : 올바른 이미지를 선택해주세요'),
                'Food photo analysis',
            );
